            ...
            for square in row:
                if square.filled:
                    ...
                    scaledPiece = pygame.transform.scale(
                        square.piece.image, (width, width)
                    )
                    position = (
                        square.position.left + square.width * 0.1,
                        square.position.top + square.width * 0.1,
                    )
...
def drawCircle(position):
    ...
    pygame.draw.circle(
        surface, color=colors.GRAY, center=(50, 50), radius=squareWidth * 0.6 / 2
    )
